 ...
 *
 * Compares row counts between public and aisafe schemas,
...
async function run() {
  ...
  console.log("  Comparing aisafe schema vs public schema");
  console.log("=".repeat(70));
  console.log();

  // 1. Discover tables in aisafe schema
  const aisafeTables = await getSchemaTableNames("aisafe");
  const publicTables = new Set(await getSchemaTableNames("public"));

  if (aisafeTables.length === 0) {
    console.log("ERROR: No tables found in aisafe schema. Nothing to verify.");
    process.exit(1);
  }

  console.log(`Found ${aisafeTables.length} tables in aisafe schema`);
  console.log(`Found ${publicTables.size} tables in public schema`);
  console.log();

  // 2. Build the list of tables to compare: aisafe tables + key tables
  const allTablesToCheck = new Set([
    ...aisafeTables,
    ...KEY_TABLES.filter((t) => aisafeTables.includes(t) || publicTables.has(t)),
  ]);
  ...
  console.log("-".repeat(70));
  console.log(
    `${"TABLE".padEnd(45)} ${"AISAFE".padStart(10)} ${"PUBLIC".padStart(10)} ${"STATUS".padStart(8)}`
  );
  console.log("-".repeat(70));
  ...
  for (const table of [...allTablesToCheck].sort()) {
    const inAISafe = aisafeTables.includes(table);
    ...
    if (inAISafe) {
      vwCount = await getRowCount("aisafe", table);
    }
    if (inPublic) {
  ...
  // 4. FK integrity checks on aisafe schema
  console.log("-".repeat(70));
  console.log("  FOREIGN KEY INTEGRITY (aisafe schema)");
  console.log("-".repeat(70));
  console.log(
  ...
  for (const [table, columns] of Object.entries(FK_MAPPINGS)) {
    for (const [column, referencedTable] of Object.entries(columns)) {
      const orphaned = await checkOrphanedFks("aisafe", table, column, referencedTable);

      if (orphaned === -1) {
        // Table(s) don't exist in aisafe schema, skip
        continue;
      }
  ...
  if (!overallPass) {
    if (mismatches > 0) {
      ...
      for (const c of comparisons.filter((c) => !c.match)) {
        console.log(
          `  - ${c.table}: aisafe=${c.aisafe_count}, public=${c.public_count}`
        );
      }
    }
