 ...
 *   2. Creates a test user (aisafe@email.com / AISafe#1)
...
async function seedTestData() {
  ...
  try {
    ...
    // 2. Create test user
    const hashedPassword = await bcrypt.hash("AISafe#1", SALT_ROUNDS);
    await sequelize.query(
      `INSERT INTO users (name, surname, email, password_hash, role_id, organization_id, created_at)
       VALUES ('Test', 'User', 'aisafe@email.com', :password, 1, :orgId, NOW())
    ...
    console.log("  Test user created: aisafe@email.com");
