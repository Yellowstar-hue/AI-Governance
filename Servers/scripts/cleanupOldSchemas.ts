 ...
 * schema after verifying that migration to the `aisafe` schema is complete.
 *
 * USAGE:
 ...
 *   4. Moves public.SequelizeMeta rows to aisafe.SequelizeMeta if needed
...
async function migrateSequelizeMeta(dryRun: boolean): Promise<void> {
  ...
  if (!verywiseHasMeta) {
    console.log(
      "  aisafe.SequelizeMeta does not exist — skipping row migration."
    );
    console.log(
  ...
  if (dryRun) {
    console.log(
      `  [DRY RUN] Would merge ${rowCount} rows from public.SequelizeMeta into aisafe.SequelizeMeta`
    );
  } else {
    // Insert rows that don't already exist in aisafe.SequelizeMeta
    await sequelize.query(
      `INSERT INTO aisafe."SequelizeMeta" (name)
       SELECT name FROM public."SequelizeMeta"
       WHERE name NOT IN (SELECT name FROM aisafe."SequelizeMeta")`,
      { type: QueryTypes.INSERT }
    );
    console.log(
      `  Merged SequelizeMeta rows from public into aisafe schema.`
    );
  }
}
...
async function run(): Promise<void> {
  ...
  try {
    ...
    // Check aisafe schema exists
    const vwExists = await verifyWiseSchemaExists();
    if (!vwExists) {
      console.error(
        "ERROR: 'aisafe' schema does not exist. Run migrations first."
      );
      process.exit(1);
    }
    console.log("Verified: 'aisafe' schema exists.\n");
