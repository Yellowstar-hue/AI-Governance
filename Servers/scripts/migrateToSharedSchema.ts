 ...
 * to the new shared aisafe schema (projects with organization_id).
 *
 * USAGE:
 ...
 *   2. For each tenant, copies data from tenant schema to aisafe schema
...
/**
 * PostgreSQL ARRAY columns in the AISAFE schema that should NOT be JSON.stringify'd
 * These need to stay as JavaScript arrays for Sequelize to convert properly
 *
 * NOTE: Only include columns where BOTH tenant and aisafe schemas use ARRAY type.
 * If aisafe schema has JSONB but tenant has ARRAY, the array needs to be stringified.
 */
...
async function getRowCount(
  ...
  if (organizationId !== undefined && schemaName === "aisafe") {
...
/**
 * Check if aisafe table has organization_id column
 */
...
/**
 * Migrate a single table from tenant schema to aisafe schema
 */
async function migrateTable(
  ...
  // Get columns that exist in BOTH source (tenant) and target (aisafe) tables
  const sourceColumns = await getTableColumns(tenantHash, tableName, transaction);
  const targetColumns = await getTableColumns("aisafe", targetTableName, transaction);
  ...
  if (sourceColumns.length !== commonColumns.length) {
    ...
    console.log(`    ℹ️  ${tableName}: skipping columns not in aisafe schema: ${skippedCols.join(', ')}`);
  }

  if (commonColumns.length === 0) {
    console.log(`    ⊘ ${tableName}: no common columns between tenant and aisafe schema`);
    return { sourceCount: 0, migratedCount: 0 };
  }
  ...
  const notNullMissing = (await sequelize.query(
    `SELECT column_name FROM information_schema.columns
     WHERE table_schema = 'aisafe' AND table_name = :tableName
    ...
    for (const row of rows) {
      ...
      for (const col of commonColumns) {
        ...
        if (fkMappings[col] && value !== null && value !== undefined) {
          ...
          // Skip remapping for struct references (meta_id columns point to aisafe struct tables)
          if (isStructReference && col.endsWith("_meta_id")) {
            // Keep original value - it references aisafe schema struct data
...
/**
 * Migrate custom framework data from tenant schema to aisafe schema
...
async function migrateCustomFrameworkPhase2(
  ...
  if (hasCFP) {
    ...
    for (const proj of projRows) {
      ...
      // Remap project_id: old tenant ID → new aisafe ID
  ...
  for (const [riskTable, implIdMap, implCol] of [
    ...
    for (const risk of riskRows) {
      ...
      // Remap risk_id: old tenant ID → new aisafe ID
...
/**
 * Migrate NIST AI RMF data from old tenant schema to aisafe schema.
 *
 * The old tenant table has inline metadata (index, title, description, category_id,
 * evidence_links, tags) while the new aisafe schema uses a struct/impl split
...
async function validateMigration(
  ...
  for (const org of organizations) {
    ...
    for (const tableName of allTables) {
      ...
      const migratedCount = await getRowCount("aisafe", tableName, undefined, org.id);
...
async function migrationStatusTableExists(): Promise<boolean> {
  const result = await sequelize.query(
    `SELECT EXISTS (
      SELECT 1 FROM information_schema.tables
      WHERE table_schema = 'aisafe' AND table_name = 'migration_status'
    ) as exists`,
    { type: QueryTypes.SELECT }
  );
  return (result[0] as any).exists;
...
// ============================================================
// COPY SHARED TABLES (public → aisafe)
// ============================================================

/**
 * Copy non-tenant-scoped tables from public → aisafe.
 * Must run BEFORE any query that reads from aisafe.organizations,
 * since search_path resolves to aisafe first.
...
async function copySharedTables(): Promise<{ tablesProcessed: number; rowsCopied: number }> {
  ...
  console.log("  Copying shared tables from public → aisafe...");
  ...
  try {
    ...
    for (const table of sharedTables) {
      ...
      const [tgtCheck] = await sequelize.query(
        `SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'aisafe' AND table_name = :table) as exists`,
        { replacements: { table }, type: QueryTypes.SELECT, transaction }
      ) as any[];
      ...
      const tgtColMeta = (await sequelize.query(
        `SELECT column_name, data_type, is_nullable FROM information_schema.columns
         WHERE table_schema = 'aisafe' AND table_name = :table ORDER BY ordinal_position`,
        { replacements: { table }, type: QueryTypes.SELECT, transaction }
      ) as any[]);
      ...
      await sequelize.query(
        `INSERT INTO aisafe."${table}" (${insertCols.join(', ')})
      ...
      if (commonCols.includes('id')) {
        await sequelize.query(
          `SELECT setval(pg_get_serial_sequence('aisafe."${table}"', 'id'), COALESCE((SELECT MAX(id) FROM aisafe."${table}"), 0))`,
          { transaction }
        );
      }

      const [insertedCount] = await sequelize.query(
        `SELECT COUNT(*) as count FROM aisafe."${table}"`,
        { type: QueryTypes.SELECT, transaction }
      ) as any[];
...
export async function migrateToSharedSchema(options: {
  ...
  try {
    // ── Step 1: Copy shared tables (public → aisafe) ──
    // Must run first so aisafe.organizations has data for the org query below.
    ...
    // ── Step 2: Discover organizations (now from aisafe via search_path) ──
...
export async function checkAndRunMigration(): Promise<MigrationResult> {
  ...
  // Copy shared tables first so aisafe.organizations has data
  await copySharedTables();

  // Now query organizations from aisafe (via search_path)
  const organizations = (await sequelize.query(
