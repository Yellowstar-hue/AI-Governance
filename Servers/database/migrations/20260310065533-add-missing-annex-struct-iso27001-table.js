...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      ...
      await queryInterface.sequelize.query(`
        CREATE TABLE IF NOT EXISTS aisafe.annex_struct_iso27001 (
      ...
      const [frameworks] = await queryInterface.sequelize.query(
        `SELECT id FROM aisafe.frameworks WHERE name = 'ISO 27001' LIMIT 1;`,
        { transaction }
      );
      ...
      if (frameworks.length > 0) {
        ...
        for (const entry of annexEntries) {
          ...
          if (inserted.length > 0) {
            ...
            await queryInterface.sequelize.query(`
              UPDATE aisafe.annexcontrols_struct_iso27001
              SET annex_id = :annexId
              FROM aisafe.annexcategories_struct_iso27001 ac
              WHERE aisafe.annexcontrols_struct_iso27001.category_id = ac.id
  ...
  async down(queryInterface, Sequelize) {
    await queryInterface.sequelize.query(`
      DROP TABLE IF EXISTS aisafe.annex_struct_iso27001;
    `);
  }
};
