 ...
 *
 * Seeds the aisafe schema struct tables with framework template data:
    ...
    try {
      ...
      for (const fw of frameworks) {
        const [existing] = await queryInterface.sequelize.query(
          `SELECT id FROM aisafe.frameworks WHERE name = :name LIMIT 1`,
          { replacements: { name: fw.name }, transaction }
        );
        if (existing.length === 0) {
          await queryInterface.sequelize.query(`
            INSERT INTO aisafe.frameworks (name, description, is_active, is_demo)
      ...
      const [fwRows] = await queryInterface.sequelize.query(
        `SELECT id, name FROM aisafe.frameworks WHERE name IN ('EU AI Act', 'ISO 42001', 'ISO 27001', 'NIST AI RMF')`,
        { transaction }
      );
      const fwIdMap = {};
      ...
      await queryInterface.sequelize.query(`DELETE FROM aisafe.nist_ai_rmf_subcategories_struct;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.nist_ai_rmf_categories_struct;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.annexcontrols_struct_iso27001;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.annexcategories_struct_iso27001;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.annex_struct_iso27001;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.subclauses_struct_iso27001;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.clauses_struct_iso27001;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.annexcategories_struct_iso;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.annex_struct_iso;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.subclauses_struct_iso;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.clauses_struct_iso;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.subcontrols_struct_eu;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.controls_struct_eu;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.controlcategories_struct_eu;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.questions_struct_eu;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.subtopics_struct_eu;`, { transaction });
      await queryInterface.sequelize.query(`DELETE FROM aisafe.topics_struct_eu;`, { transaction });
      ...
      if (!euFrameworkId) {
        ...
        for (const topic of Topics) {
          const topicId = await insertReturningId(`
            INSERT INTO aisafe.topics_struct_eu (framework_id, title, order_no, is_demo)
          ...
          for (const subtopic of topic.subtopics) {
            const subtopicId = await insertReturningId(`
              INSERT INTO aisafe.subtopics_struct_eu (topic_id, title, order_no, is_demo)
            ...
            for (const q of subtopic.questions) {
              await queryInterface.sequelize.query(`
                INSERT INTO aisafe.questions_struct_eu (subtopic_id, order_no, question, hint, priority_level, answer_type, input_type, evidence_required, is_required, is_demo)
        ...
        for (const category of ControlCategories) {
          const categoryId = await insertReturningId(`
            INSERT INTO aisafe.controlcategories_struct_eu (framework_id, title, order_no, is_demo)
            VALUES (:frameworkId, :title, :order_no, false)
            ON CONFLICT DO NOTHING
            RETURNING id
          ...
          for (const control of category.controls) {
            const controlId = await insertReturningId(`
              INSERT INTO aisafe.controls_struct_eu (control_category_id, title, description, order_no, is_demo)
            ...
            for (const sc of control.subControls) {
              await queryInterface.sequelize.query(`
                INSERT INTO aisafe.subcontrols_struct_eu (control_id, title, description, order_no, is_demo)
      ...
      if (!iso42001FrameworkId) {
        ...
        for (const clause of Clauses) {
          const clauseId = await insertReturningId(`
            INSERT INTO aisafe.clauses_struct_iso(framework_id, clause_id, title, clause_no, order_no, is_demo)
          ...
          for (const sub of clause.subclauses) {
            await queryInterface.sequelize.query(`
              INSERT INTO aisafe.subclauses_struct_iso(clause_id, subclause_id, title, summary, questions, evidence_examples, order_no, is_demo)
        ...
        for (const annex of Annex) {
          const annexId = await insertReturningId(`
            INSERT INTO aisafe.annex_struct_iso(framework_id, title, annex_no, order_no, is_demo)
          ...
          for (const cat of annex.annexcategories) {
            await queryInterface.sequelize.query(`
              INSERT INTO aisafe.annexcategories_struct_iso(annex_id, sub_id, title, description, guidance, order_no, is_demo)
      ...
      if (!iso27001FrameworkId) {
        ...
        for (const clause of ISO27001Clause) {
          const clauseId = await insertReturningId(`
            INSERT INTO aisafe.clauses_struct_iso27001 (framework_id, clause_id, title, order_no, is_demo)
          ...
          for (const sub of clause.subclauses) {
            await queryInterface.sequelize.query(`
              INSERT INTO aisafe.subclauses_struct_iso27001 (clause_id, subclause_id, title, requirement_summary, key_questions, evidence_examples, order_no, is_demo)
        ...
        for (const annex of ISO27001Annex) {
          ...
          const annexStructId = await insertReturningId(`
            INSERT INTO aisafe.annex_struct_iso27001 (framework_id, arrangement, title, order_no, is_demo)
          ...
          const annexCategoryId = await insertReturningId(`
            INSERT INTO aisafe.annexcategories_struct_iso27001 (framework_id, annex_id, title, order_no, is_demo)
          ...
          for (const ctrl of annex.controls) {
            await queryInterface.sequelize.query(`
              INSERT INTO aisafe.annexcontrols_struct_iso27001 (category_id, annex_id, control_id, title, order_no, requirement_summary, key_questions, evidence_examples, is_demo)
        ...
        for (const func of NIST_AI_RMF_Structure.functions) {
          ...
          for (const category of func.categories) {
            const categoryStructId = await insertReturningId(`
              INSERT INTO aisafe.nist_ai_rmf_categories_struct (framework_id, function, category_id, description, order_no, is_demo)
            ...
            for (const sub of category.subcategories) {
              await queryInterface.sequelize.query(`
                INSERT INTO aisafe.nist_ai_rmf_subcategories_struct (category_struct_id, function, subcategory_id, description, order_no, is_demo)
