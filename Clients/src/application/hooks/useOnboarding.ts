...
import { AISafeContext } from "../contexts/AISafe.context";
import { useAuth } from "./useAuth";
import { setOnboardingStatus as setReduxOnboardingStatus } from "../redux/auth/authSlice";
import type { RootState } from "../redux/store";

const getStorageKey = (userId: number) => `aisafe_onboarding_${userId}`;
...
export const useOnboarding = () => {
  ...
  const { users, organizationId } = useContext(AISafeContext);
